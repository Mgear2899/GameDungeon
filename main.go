...
func main() {
	...
	// Инициализация БД
	db, err := sql.Open("sqlite3", "./dungeon_bot.db")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func handleMessage(bot *tgbotapi.BotAPI, db *sql.DB, message *tgbotapi.Message) {
	...
	case "Лечение":
		if potion(message.Chat.ID, 1) < 3 {
			handleHeal(bot, db, player, message.Chat.ID)
		}
		// itemsMenu(bot, db, message.Chat.ID, message, player)
	case "Убежать":
		monster = getMonsterLite(player.Stage)
		handleRun(bot, db, player, monster, message.Chat.ID, step)
	case "help":
...
func handleAttack(bot *tgbotapi.BotAPI, db *sql.DB, player *Player, monster *Monster, chatID int64, message *tgbotapi.Message) {
	...
	if uph.state {
		...
		if uph.hpmonster > 0 {
			...
			var item string
			r := randItems()
			sel := selectItems(db, strconv.Itoa(r), int(chatID))
			if sel.Name != "" {
				count := sel.Count + 1
				updateItems(db, strconv.Itoa(r), int(chatID), count)
				sel := selectItems(db, strconv.Itoa(r), int(chatID))
				item = fmt.Sprintf("Получен предмет: %s(x%d)", sel.Name, sel.Count)
			} else {
				i := items[strconv.Itoa(r)]
				add := addItems(db, strconv.Itoa(r), int(chatID))
				if add != "" {
					item = fmt.Sprintf("Получен предмет: %s(x%d)", i.Name, i.Count)
				}
			}

			msg = tgbotapi.NewMessage(chatID, fmt.Sprintf("Вы победили %s!\nЗолото: %d, Опыт: %d\n%s",
				monster.Name, player.Gold, player.XP, item))
			bot.Send(msg)
...
func handleHeal(bot *tgbotapi.BotAPI, db *sql.DB, player *Player, chatID int64) {
	...
	player.HP += rand.Intn(20) + 13*player.Level/5
	updatePlayer(db, player)
...
func handleRun(bot *tgbotapi.BotAPI, db *sql.DB, player *Player, monster *Monster, chatID int64, step int) {
	if step >= 5 {
...
func getMonsterLite(stage int) *Monster {
	monsters := []Monster{
		...
		{"Орк", 70, 10, 7, 8, 5},
		{"Мумия", 75, 13, 6, 6, 6},
		{"🧟‍♀️ Вурдолак", 45, 4, 8, 7, 4},
		{"💀 Скелет воин", 65, 11, 9, 8, 2},
	}
...
func getMonsterBoss(stage int) *Monster {
	monsters := []Monster{
		{"Дракон", 260, 20, 20, 20, 15},
		{"Дракула", 200, 27, 15, 17, 9},
		{"Троль", 210, 29, 17, 26, 8},
		{"Минотавр", 250, 35, 18, 28, 6},
		// Name:     "",
		// HP:       0,
		// AtkPower: 0,
		// Def:      0,
...
func calculateDamage(attacker Class, target Monster) (int, string) {
	...
	critChance := 0.2 // 20% шанс на крит
